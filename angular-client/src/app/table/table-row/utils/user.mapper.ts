...
export const outputKeyMap = {
    ...
    "roleKey":(user:User,key:string)=>{
        return Role[(user as any).roleKey];
    },
...
export const editKeyMap = {
    ...
    "roleKey":(user:User,key:string,index:number)=>{
        ...
        <option value="0" ${(user as any).roleKey==0?'selected':""} >${Role[0]}</option>
        <option value="1" ${(user as any).roleKey==1?'selected':""} >${Role[1]}</option>
        <option value="2" ${(user as any).roleKey==2?'selected':""} >${Role[2]}</option>
      </select>`
    },
